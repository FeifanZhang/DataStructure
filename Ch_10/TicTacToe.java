...
//10.23 and 10.25
protected int minimax(int step){
	return miniMax(true,step);
}

protected int miniMax(boolean that,int step){
	int score=score();
	if (gameOver()||step==0) {
		   return score;
		 }
	...
	}for (int row = 0; row < 3; row++) 
	{
		   for (int column = 0; column < 3; column++) 
		   {
			     if (squares[row][column] == '.') 
			     {
			    	 if(that==true)squares[row][column] = 'O';
			    	 else squares[row][column] = 'X';
			       score = miniMax(!that,step-1);
			       if(that==true){
			    	   if (score < bestScore) {
			    	         bestScore = score;
			    	       }
