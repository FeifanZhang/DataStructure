...
{
	outputDecisionTree(this.root,0);
}
private void outputDecisionTree(BinaryNode that,int i)
{
	BinaryNode<String> node=that;
	...
	if(node.getLeft()!=null)outputDecisionTree(node.getLeft(),i+2);
	if(node.getRight()!=null)outputDecisionTree(node.getRight(),i+2);
	return;
}
